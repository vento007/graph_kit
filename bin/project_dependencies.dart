...
void main() {
  ...
  // Frontend services
  graph.addNode(
    Node(id: 'web_app', type: 'Frontend', label: 'Web Application'),
  );
  ...
  graph.addNode(
    Node(id: 'auth_service', type: 'Service', label: 'Authentication Service'),
  );
  graph.addNode(
    Node(id: 'user_service', type: 'Service', label: 'User Management'),
  );
  graph.addNode(
    Node(id: 'order_service', type: 'Service', label: 'Order Processing'),
  );
  graph.addNode(
    Node(id: 'payment_service', type: 'Service', label: 'Payment Gateway'),
  );
  graph.addNode(
    Node(id: 'notification_service', type: 'Service', label: 'Notifications'),
  );

  // Databases
  graph.addNode(Node(id: 'user_db', type: 'Database', label: 'User Database'));
  graph.addNode(
    Node(id: 'order_db', type: 'Database', label: 'Order Database'),
  );
  graph.addNode(
    Node(id: 'analytics_db', type: 'Database', label: 'Analytics Database'),
  );

  // External services
  graph.addNode(
    Node(id: 'stripe', type: 'External', label: 'Stripe Payment API'),
  );
  graph.addNode(
    Node(id: 'sendgrid', type: 'External', label: 'SendGrid Email API'),
  );
  ...
  for (final dbId in databases) {
    ...
    print(
      '      Total impact: ${dbUsers.nodes.length - 1} services',
    ); // -1 to exclude the DB itself
  ...
  for (var i = 0; i < layers.length; i++) {
    print(
      '   Layer $i: ${layers[i].map((id) => graph.nodesById[id]?.label).join(', ')}',
    );
  }
}
...
void _printDependents(
  String rootId,
  SubgraphResult dependents,
  Graph<Node> graph,
) {
  final root = graph.nodesById[rootId];
  ...
  for (final nodeId in dependents.nodes) {
    if (nodeId != rootId) {
      ...
      print(
        '   ${distance == 1 ? '├─' : '└─'} $icon ${node?.label} ${distance > 1 ? '(indirect)' : '(direct)'}',
      );
    }
  }
}

void _printImpactAnalysis(
  String serviceId,
  SubgraphResult impact,
  Graph<Node> graph,
) {
